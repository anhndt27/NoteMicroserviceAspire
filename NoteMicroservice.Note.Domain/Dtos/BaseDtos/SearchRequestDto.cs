...
using NoteMicroservice.Note.Domain.Constants;
...
namespace NoteMicroservice.Note.Domain.Dtos.BaseDtos
{
    public abstract class SearchRequestDto<TModel> where TModel : BaseModel
    {
        ...
        public abstract bool TryCreateSingleQuery(NoteDbContext context, out IQueryable<TModel> query);
            ...
            try
            {
                ...
                switch (OrderBy)
                {
                    ...
                    case "CreatedByUser":
                        ...
                        {
                            query = query.OrderBy(OrderByDesc, e => ((IUserTrackableModels)e).CreatedByUserId );
                        }
                        break;
                    case "UpdatedTimeUtc":
                        if (typeof(ITimeTrackableModel).IsAssignableFrom(typeof(TModel)))
                    ...
                    case "UpdatedByUser":
                        if (typeof(IUserTrackableModels).IsAssignableFrom(typeof(TModel)))
                        {
                            query = query.OrderBy(OrderByDesc, e => ((IUserTrackableModels)e).UpdatedByUserId);
                        }
                        break;
                }
